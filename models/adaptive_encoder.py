...
class AdaptiveSteganographyEncoder(nn.Module):
    ...
    def _adjust_message_size(self, message):
        """Simple adapter to resize messages - no gradient tracking needed"""
        ...
        if message.size(1) > self.fixed_message_length:
            # Truncate
            return message[:, :self.fixed_message_length]
        else:
            # Pad with zeros
            padding = torch.zeros(message.size(0), 
                                 self.fixed_message_length - message.size(1),
    ...
    def forward_frequency(self, image, message, feature_weights):
        ...
        stego_image = self.freq_embedding(image, adjusted_message, feature_weights)
        
        return stego_imageimport torch
import torch.nn as nn
import torch.nn.functional as F
from torch.fft import rfft2, irfft2
